...
def SubBytesInverse(state):
    subState = []
    for row in state:
        subState.append(SubwordInverse(row))

    return subState


def RotwordInverse(word):
    if type(word) != int:
        raise ValueError("Le mot en entrée doit être un entier")
    elif word > 0xffffffff:
        raise ValueError("Le mot en entrée doit être un entier sur 8 octets")
...
def ShiftRowInverse(state):
    ...
    for row in rows:
        ...
        for _ in range(rotate_times):
            rotated = RotwordInverse(rotated)
...
def MixColumnsInverse_calcul(column):
...
def MixColumnsInverse(state):
    ...
    for column in state:
        mixed = MixColumnsInverse_calcul(column)
        result.append(mixed)
